...
function MyForm() {
  ...
  const [disappearedNumbers, setDisappearedNumbers] = useState([]); // State để lưu các số đã biến mất
  ...
  const handleStartClick = () => {
    ...
    if (!isNaN(parsedValue) && parsedValue > 0) {
      ...
      setDisappearedNumbers([]); // Reset các số đã biến mất
...
{/* Hiển thị thông báo */}
        ...
        >
          ...
          {numbers.map((number, index) => {
            ...
            return !disappearedNumbers.includes(number) ? (
              <span
                key={number}
                onClick={() => handleNumberClick(number)} // Xử lý khi nhấn vào số
                className={fadingNumbers.includes(number) ? "fade-out" : ""}
                style={{
